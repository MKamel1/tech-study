...
- [Company-Specific Angles](#company-specific-angles)
  - [Causal Inference × Time Series Bridge](#cross-domain-causal-inference--time-series) *(NEW)*
- [Code Memorization Priority Guide](#code-memorization-priority-guide)
- [Interview Cheat Sheet](#interview-cheat-sheet)
- [Self-Test Questions](#self-test-questions)
  - [Senior AS Questions](#senior-as-questions-organizational--communication-dimension) *(NEW)*
...
### Cross-Domain: Causal Inference × Time Series

> [!TIP]
> **Why This Matters**: Causal inference is the primary spike in your AS study agenda. Time series and causal inference intersect constantly in supply chain and economics roles. Knowing the bridges makes you look like a senior candidate who thinks across domains.

This is not a company-specific angle — it appears in interviews at Amazon (supply chain causal effects), Meta (experimentation), and any economics-adjacent AS role.

#### Bridge 1: Interrupted Time Series (ITS) = ARIMAX with an Intervention Indicator

When a policy, price change, or event happens at a known point in time, you can model its effect using a before-after framework in ARIMAX.
...
# Add a binary indicator for the intervention (e.g., price cut on 2023-06-01)
df['intervention'] = (df['date'] >= '2023-06-01').astype(int)

# Optionally add a "ramp" to capture gradual effects
df['post_ramp'] = df['intervention'] * np.arange(len(df))

model = SARIMAX(df['sales'], exog=df[['intervention', 'post_ramp']], order=(1,1,1))
result = model.fit()

# The coefficient on 'intervention' = estimated level shift effect
# The coefficient on 'post_ramp' = estimated gradual drift per period
print(result.params[['intervention', 'post_ramp']])
```

**The causal caveat**: The intervention coefficient is only causal if:
- No confounders changed at the same time (parallel trends assumption for the ARIMA residuals)
- The series was not already trending toward the post-intervention level

This is exactly the **DiD (Difference-in-Differences) logic** applied to a single time series. The ARIMA component controls for autocorrelation; the indicator estimates the treatment effect.

#### Bridge 2: Synthetic Control = Forecast Counterfactual

The **Synthetic Control Method** (Section 1.2.5 in your causal inference plan) IS a forecasting problem:

1. **Pre-treatment period**: Fit a weighted combination of donor series that matches the treated series
2. **Treatment period**: The "forecast" from step 1 is the counterfactual — *what would have happened without treatment*
3. **Causal effect** = actual outcome minus the forecast/counterfactual

The quality of the causal estimate depends entirely on the quality of the pre-treatment forecast. A better forecasting model = a better causal estimate. This is why forecasting skills are directly transferable to causal inference.

```python
# Conceptual: synthetic control is a constrained forecasting problem
# Minimize: sum((Y_treated_pre - w @ Y_donors_pre)^2)
# Subject to: w >= 0, sum(w) = 1
# Then: counterfactual = w @ Y_donors_post

from scipy.optimize import minimize

def synthetic_control_weights(Y_treated_pre, Y_donors_pre):
    """Find optimal donor weights to match pre-treatment series."""
    n_donors = Y_donors_pre.shape[1]
    w0 = np.ones(n_donors) / n_donors

    result = minimize(
        lambda w: np.sum((Y_treated_pre - Y_donors_pre @ w)**2),
        w0,
        method='SLSQP',
        bounds=[(0, 1)] * n_donors,
...
#### Bridge 3: DiD Uses Time Series Structure

**Difference-in-Differences** (Section 1.2.2 in your causal plan) is built on a time series comparison:

- **First difference**: Post vs pre for the treated group (removes time-invariant confounders)
- **Second difference**: Subtract the first difference of the control group (removes common time trends)

The **parallel trends assumption** is fundamentally a time series claim: *"In the absence of treatment, the treated group would have followed the same trend as the control group."* Forecasting methods can be used to **test** whether pre-treatment trends are parallel and to **construct** the counterfactual trend.

#### Bridge 4: Granger Causality vs True Causality

Granger causality (covered in Chapter 02) is a temporal precedence test — not a causal test. The correct causal question is answered by one of: A/B testing, IV, DiD, or synthetic control. Granger causality is useful for feature selection and lead-lag discovery, but an interviewer from an economics background will probe whether you understand the distinction.

| Tool | What It Tells You | Causal? |
|------|--------------------|---------|
| Granger causality test | Does X help predict Y? | No — correlation only |
| ARIMAX with intervention | Did the intervention shift the level? | Only under parallel trends |
| Synthetic control | Counterfactual if intervention hadn't happened | Yes, with assumptions |
| A/B test / RCT | Randomized comparison | Yes (gold standard) |

#### Interview Sample Questions

**Q**: *"How would you estimate the demand uplift from a promotion?"*

**A**: "I'd frame this as an interrupted time series problem. I'd fit ARIMAX with a promotion indicator on the pre-promotion + promotion period data. The coefficient on the indicator gives the level shift estimate. I'd validate that no other changes happened simultaneously (confounder check) and report a credible interval around the estimate. For a cleaner causal estimate, I'd prefer a holdout A/B test where some markets got the promotion and some didn't — that gives us the counterfactual directly."

**Q**: *"What's the difference between Granger causality and true causality in the context of your demand model?"*

**A**: "Granger causality means past price values help forecast future demand beyond what past demand alone could forecast — it's a predictive, temporal relationship. True causality would require that we isolated the effect of price changes on demand while holding everything else constant — through a randomized price experiment, an IV, or a natural experiment. Granger causality could be spurious due to common causes (e.g., seasonal patterns affecting both) or reverse causation (demand changes may have triggered the price change)."
...
### M-Competition Empirical Findings (Know These — Senior-Level Evidence)

> [!TIP]
> Citing forecasting competition results in an interview signals empirical awareness. These are the most interview-relevant findings.

| Competition | Key Finding | Interview Implication |
|-------------|-------------|----------------------|
| **M1 (1982)** | Simple methods (ETS) competitive with complex models | "Don't default to complex models; validate with a simple baseline" |
| **M3 (2000)** | Combination of methods outperformed individual models; damped trend ETS won | "I ensemble because the M3 showed single models rarely win" |
| **M4 (2018)** | Hybrid statistical + ML model (ES-RNN) won; simple average combinations beat most individuals; MASE was the primary metric | "M4 taught us that combining statistical structure with ML residuals beats either alone; and that MASE is the right metric" |
| **M5 (2020)** | Global LightGBM dominated; Walmart demand data at scale; cross-series learning won | "M5 validated global models for large-scale retail — one LightGBM trained on all SKUs beat per-SKU ARIMA" |

**The most important empirical finding to cite**: *"Across M1 through M5, simple combinations of diverse models consistently outperformed any single model, no matter how sophisticated. The practical implication is that I always build an ensemble of at least 2-3 models and combine them — the M4 winner used a simple average of 8 diverse methods alongside the ES-RNN."*
...
### Senior AS Questions (Organizational + Communication Dimension)

<details>
<summary><strong>Q: Your Prophet model backtests well (MASE=0.82) but the operations team refuses to use the forecasts. What do you do?</strong></summary>

**Answer**:
This is a trust and adoption problem, not a modeling problem. Strong backtest metrics mean the model is technically sound — the issue is that the operations team doesn't understand or believe the output.

**Step 1: Diagnose the distrust.** Have a conversation. Common reasons:
- The model failed on a memorable recent event (a stockout, a big miss on a promotional period)
- The forecasts seem too "smooth" or "robotic" — no human judgment is incorporated
- The output format doesn't map to their workflow

**Step 2: Address specifically**:
- If it failed on a recent event: Show the model with and without that event as a known intervention. Explain what the model can and can't do.
- If it's smooth/robotic: Add a human override mechanism. Allow the team to adjust forecasts and track which overrides improve vs hurt accuracy. Build an override audit trail.
- If it's format: Deliver forecasts in their preferred tool (Excel, their ERP system) with familiar column names.

**Step 3: Build evidence gradually.** Start with a shadow-mode pilot: run the model forecasts alongside the existing process for 8–12 weeks, compare outcomes, present the analysis. Let the data make the case.

**Senior framing**: A model that isn't used has zero business value, regardless of MASE. Part of the AS role is change management — you're responsible for adoption, not just accuracy.

</details>

<details>
<summary><strong>Q: You've been asked to improve forecast accuracy by 20% in Q1. How do you respond?</strong></summary>

**Answer**:
I'd push back on the framing before agreeing to the target.

**Clarifying questions I'd ask**:
1. "What is the current MASE / WAPE? Is 20% improvement against the naive baseline, or the current model?"
2. "Which SKUs / segments are we measuring this on? The 80/20 rule applies — 20% improvement on tail items may be meaningless."
3. "What is the current model? Is this a first-generation model or a mature one?"
4. "Why 20%? What business outcome does that accuracy improvement drive — is it inventory reduction, service level, or fill rate?"

**Then I'd frame an honest answer**:
- If the current model is unsophisticated (naive seasonal or simple ETS), 20% improvement is plausible through proper ML feature engineering and walk-forward validation.
- If the current model is already a global LightGBM with rich features, 20% is extremely ambitious — the remaining error is likely irreducible noise. I'd propose a 5–10% target and show empirical evidence of the noise floor.
- In any case, I'd decompose error into: systematic (model can fix), external (exogenous shocks the model can't see), and irreducible (demand randomness). Only the first category is improvable.

**Senior framing**: Agreeing blindly to a 20% target without this analysis sets the team up for failure. The right answer is: "Let me run a decomposition of current forecast error into fixable vs irreducible components, then give you an honest improvement target with a confidence interval."

</details>

<details>
<summary><strong>Q: Your model has been running in production for 6 months and suddenly has 40% MAPE last week. What's your diagnostic process?</strong></summary>

**Answer**:
A sudden spike from normal to 40% MAPE suggests a structural event, not a gradual drift. I'd follow this diagnostic sequence:

**Step 1: Check the data pipeline first (90% of production issues are data, not models)**
- Did the input data arrive late, truncated, or with schema changes?
- Was there a data source switch (new ERP system, different aggregation logic)?
- Were there unusual imputed or missing values in the training or scoring data?

**Step 2: Check for an external event**
- Was there a major business event? Product launch, competitor action, supply disruption?
- Is the MAPE concentrated in specific SKUs, categories, or regions? A localized spike points to an event.

**Step 3: Diagnose model drift**
- Check the residual distribution for the past 4 weeks. Is the bias now consistently positive or negative (systematic miss)?
- Plot the features the model used — have any feature distributions shifted? (Data drift)
- Check if a model feature is now returning null or default values.

**Step 4: Immediate response**
- If a data issue: fix the pipeline, retrain on clean data, backfill.
- If an external structural event: add an intervention indicator, consider truncating training data.
- If model drift: trigger an unscheduled retraining.

**Prevention**: I'd design the production system with: (a) automated data quality checks that alert before scoring, (b) a rolling MAPE monitor that pages when >2σ above baseline, (c) scheduled weekly retraining with a champion-challenger framework.

**Senior framing**: 40% MAPE is almost certainly not a model issue on week 1. It's almost always a data or event issue. Jumping to "let's rebuild the model" before diagnosing the data is a common junior mistake.
